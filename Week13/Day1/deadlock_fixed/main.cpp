...
void equip1() {
    while(true){
        unique_lock<mutex> swordLock(swordMutex, defer_lock);
        ...
        if (swordLock.try_lock()) {
            ...
            this_thread::sleep_for(chrono::milliseconds(2000)); // Simulate some processing time
...
void equip2() {
    while(true) {
        unique_lock<mutex> shieldLock(shieldMutex, defer_lock);
